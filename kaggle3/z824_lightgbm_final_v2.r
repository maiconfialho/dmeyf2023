...
for (i in seq_along(semillas)) {
    ...
    for (envios in cortes) {
        ...
        # Bagging: se realiza el promedio de las predicciones de varios modelos
        for (bagging_iter in seq_along(semillas)) {
            modelo_bagging <- lgb.train(
                data = dtrain,
                param = param_completo,
            )
        ...
        tb_entrega[, prob := tb_entrega$prob / length(semillas)]  # Se promedian las predicciones

        # Asignar 1 a los mejores envíos
        tb_entrega[1:envios, Predicted := 1L]

        # Guardar archivo de envío
        fwrite(tb_entrega[, list(numero_de_cliente, Predicted)],
        file = paste0(PARAM$experimento, "_", semilla, "_", envios, ".csv"),
        sep = ","
        )
    }
}
